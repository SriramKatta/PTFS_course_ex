...
int main(int argc, char **argv)
{
  ...
  // dummy parallel region to remove pinning overhead
#pragma omp parallel
  {
    if (omp_get_num_threads() == 0)
    {
      printf("Error: Please set the number of threads to more than 1\n");
      exit(1);
    }
  }
...
#pragma omp single 
    for (yc = 0; yc < xtiles; yc++)
      ...
      {
#pragma omp task firstprivate(xc, yc) private(tile) 
        {
